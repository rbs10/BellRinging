    ...
    public class CyclicSearch2
    {
        ...
        public static void Checks()
        {
                ...
                methodLibrary.MethodNames)
            {
                try
                {
                    var search = new CyclicSearch2(method);

                    System.Diagnostics.Debug.WriteLine("{0} = {1}", method, "OK");
                    Console.WriteLine("{0},{1},{2},{3}",method,search.rows.Count,musicalPreferences.ScoreLead(search.rows),
                        search.method.WrongPlaceCount);
       ...
       static IEnumerable<KeyValuePair<Row,string>> Music(IEnumerable<Row> rows)
        ...
        public CyclicSearch2(string methodName)
        {
            Check(methodName);
        }
